...
export const ConfirmDialog: React.FC<ConfirmDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div
        className="bg-surface-main border-border-main bg-white w-full max-w-sm rounded-lg border shadow-lg p-6 animate-in fade-in zoom-in-95 duration-200"
        role="dialog"
        aria-modal="true"
      >
        <h3 className="text-text-main text-lg font-semibold mb-2">{title}</h3>
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className={`rounded px-4 py-2 text-sm font-bold text-white transition-colors shadow-sm ${
              variant === "danger"
