...
import model.elements.Math.{Rational, mod}
...
case class Interval(octave: Int, fifth: Int) {
  ...
  def -(that: Interval): Interval = Interval(octave - that.octave, fifth - that.fifth)
  def *(i: Int): Interval         = Interval(octave * i, fifth * i)

  def normalize: Interval = {
    val s = step
    if (s.value > 0) Interval.fromStepAlter(IntervalStep(s.value % 7), alter)
    else if (s.value < 0) Interval.fromStepAlter(IntervalStep((-1 * s.value) % 7), alter)
    else this
  }
  ...
  lazy val stepAlter: (IntervalStep, IntervalAlter) = {
    ...
    val absFifth = scala.math.abs(fifth)
  ...
  def abs: Interval = {
    val s = step.value
    if (s >= 0) this else Interval(-octave, -fifth)
  }
...
object Interval {
  ...
  def of(base: Pitch, target: Pitch): Interval = target - base

  /** 音程の「⚪︎度上」を表す。ただし 0-indexed で表し、「一度」の値は 0 となる。 */
  case class IntervalStep(value: Int) extends Ordered[IntervalStep] {
    def +(other: IntervalStep): IntervalStep = IntervalStep(this.value + other.value)
    def -(other: IntervalStep): IntervalStep = IntervalStep(this.value - other.value)
    def *(i: Int): IntervalStep              = IntervalStep(this.value * i)
    def abs: IntervalStep                    = IntervalStep(scala.math.abs(this.value))
    def toIdx1: Int                          = if (value >= 0) value + 1 else value - 1

    override def compare(that: IntervalStep): Int = this.value compare that.value
  }
  ...
  lazy val d1 = parse("d1")
  lazy val A1 = parse("A1")
  lazy val A2 = parse("A2")
  lazy val m3 = parse("m3")
  lazy val M3 = parse("M3")
  lazy val d4 = parse("d4")
  lazy val A4 = parse("A4")
  lazy val d5 = parse("d5")
  lazy val P5 = parse("P5")
  lazy val A5 = parse("A5")
  lazy val m6 = parse("m6")
  lazy val M6 = parse("M6")
  lazy val A6 = parse("A6")
...
object Key {
  ...
  def parse(name: String): Key = {
    val parts = name.split(" ")
    if (parts.length != 2) throw new IllegalArgumentException(s"Invalid key format: $name")
    Key(Pitch.NoteName.parse(parts(0)), Mode.parse(parts(1)))
  }
  ...
  def calculateScalePitch(keyTonicVal: Int, modeOffset: Int, intervalStepVal: Int): (Int, Int) = {
    ...
    val actualStepVal = intervalStepVal

    def getAlters(num: Int): List[Int] = {
      val q = java.lang.Math.floorDiv(num, 7)
      val r = mod(num, 7)
...
/** 音度。調内における相対的な位置を表す。
  ...
  */
case class Degree(step: Degree.DegreeStep, alter: Degree.DegreeAlter) extends Ordered[Degree] {

  def noteName(key: Key): Pitch.NoteName = {
    val (rootStep, _)    = InternationalPitch.fromNoteName(key.tonic)
    val (_, diatonicVal) = Key.calculateScalePitch(key.tonic.value, key.mode.offset, step.value + rootStep.ordinal)
...
object Degree {
  ...
  case class DegreeStep(value: Int) extends Ordered[DegreeStep] {
    require(0 <= value && value <= 6, "Step must be 0-6")

    def +(that: DegreeStep): DegreeStep = DegreeStep((this.value + that.value) % 7)
    def -(that: DegreeStep): DegreeStep = DegreeStep(mod(this.value - that.value, 7))

    override def compare(that: DegreeStep): Int = this.value compare that.value

    def toIdx1: Int = value + 1
  }

  object DegreeStep {
    def idx1(step: Int): DegreeStep = DegreeStep(step - 1)
  }

  /** 音度の変化記号部分（変化なし=0）。 */
  case class DegreeAlter(value: Int) extends Ordered[DegreeAlter] {
    require(-1 <= value && value <= 2, "Alter must be -1 to 2")

    override def compare(that: DegreeAlter): Int = this.value compare that.value
  }
  ...
  def fromNoteNameKey(noteName: Pitch.NoteName, key: Key): Degree = {
    ...
    val stepDiff   = mod(nStep.ordinal - tStep.ordinal, 7).toInt
    val degreeStep = DegreeStep(stepDiff)

    val (_, diatonicVal) = Key.calculateScalePitch(key.tonic.value, key.mode.offset, degreeStep.value + tStep.ordinal)
    val diff             = noteName.value - diatonicVal

    val alterVal = diff / 7
    Degree(degreeStep, DegreeAlter(alterVal))
  }

  def idx1(step: Int, alter: Int): Degree = Degree(DegreeStep.idx1(step), DegreeAlter(alter))
}
...
case class Duration(value: Rational) extends Ordered[Duration] {
  ...
  def -(that: Duration): Duration           = Duration(this.value - that.value)
  def *(i: Long): Duration                  = Duration(this.value * i)
  def *(i: Rational): Duration              = Duration(this.value * i)
  override def compare(that: Duration): Int = this.value.compare(that.value)
  override def toString: String             = s"d=${value.toMixedNumberString}"

  def asOffset: Offset = Offset(this.value)
}

object Duration {
  def of(numerator: Long, denominator: Long = 1): Duration = Duration(Rational(numerator, denominator))

  def parse(s: String): Duration = Duration(Rational.parse(s))
}
...
case class Offset(value: Rational) extends Ordered[Offset] {
  ...
  def -(that: Offset): Duration           = Duration(this.value - that.value)
  def -(that: Duration): Offset           = Offset(this.value - that.value)
  def *(i: Long): Offset                  = Offset(this.value * i)
  def *(i: Rational): Offset              = Offset(this.value * i)
  override def compare(that: Offset): Int = this.value.compare(that.value)
  override def toString: String           = s"Offset(${value.toMixedNumberString})"
...
object Offset {
  ...
  def idx1(numerator: Int, denominator: Int = 1): Offset = Offset(Rational(numerator - 1, denominator))

  def parse(s: String): Offset = Offset(Rational.parse(s))
}

/** スコア上のある時間の範囲。start: inclusive, end: exclusive */
case class Span(start: Offset, end: Offset) {
  assert(start < end, s"illegal value of span. start: ${start}, end: ${end}")

  override def toString: String = {
    s"Span(${start.value.toMixedNumberString} ~ ${end.value.toMixedNumberString})"
  }
}

object Span {
  def parse(startStr: String, endStr: String): Span = {
    val startOffset = Offset.parse(startStr)
    val endOffset   = Offset.parse(endStr)
    Span(startOffset, endOffset)
  }
}

/** スコア上のパートと時間の範囲 */
case class Scope(parts: Set[Part], span: Span)

/** 楽曲のパート情報。
  *
  * "Soprano" :: "voice1" :: Nil のように階層化して管理する。
  *
  * TODO: 順序をこの構造の中に埋め込んでしまうか？
  */
case class Part(hierarchy: List[String]) extends Ordered[Part] {

  def spawn(childName: String): Part = Part(hierarchy :+ childName)

  // Part.of("Soprano") isSupersetOf Part.of("Soprano", "voice1") == true
  def isSupersetOf(descendant: Part): Boolean = {
    descendant.hierarchy.startsWith(this.hierarchy)
  }

  override def toString: String = if (this == Part.Root) { "Root" }
  else { hierarchy.mkString(":") }

  override def compare(that: Part): Int = {
    // 階層の要素を先頭から順に比較する
    // P1:voice1000 vs P2:voice2000 -> P1 vs P2 で決まる
    // P1 vs P1:voice1000 -> P1 が先（短い方が先）
    val zipped = this.hierarchy.zip(that.hierarchy)
    zipped
      .map { case (thisElem, thatElem) =>
        thisElem.compare(thatElem)
      }
      .find(_ != 0)
      .getOrElse {
        // 共通部分が全て一致する場合、階層が浅い方を先とする
        this.hierarchy.size.compare(that.hierarchy.size)
      }
  }
...
object Part {
  ...
  /** ルート（階層なし）を表す定数 */
  val Root: Part = Part(Nil)

  def of(name: String*): Part = Part(name.toList)

  /** 指定された複数のパートに共通する親（最小共通祖先）を見つけて返す。
    * * 例:
    * - [A:B:C, A:B:D] -> A:B
    * - [A:B, A]       -> A
    * - [A:B, X:Y]     -> (Root)
    * * @param parts 共通の親を探したいパートの集合
    * @return 共通の親パート。共通部分がない場合は Part(Nil) (Root) を返す。
    */
  def commonAncestor(parts: Iterable[Part]): Part = {
    if (parts.isEmpty) return Root

    // 最初のパートの階層を基準に、他の全パートとの共通部分を計算する
    val commonHierarchy = parts.map(_.hierarchy).reduceLeft { (acc, current) =>
      acc
        .zip(current)
